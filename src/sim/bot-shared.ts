...
export function specialsOf(cards: Card[], kind: SpecialCard["special"]): SpecialCard[] {
  return cards.filter((c): c is SpecialCard => c.kind === "special" && c.special === kind);
}

export function subsets(cards: Card[], min: number, max: number): Card[][] {
  const out: Card[][] = [];
...
export function pickEchangeTarget(state: GameState, playerId: number, rng: () => number): number {
